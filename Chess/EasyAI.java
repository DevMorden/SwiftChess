...
public class EasyAI implements ChessPlayer{
    ...
    // Public methods
    // This method is used to make a valid move we want the computer to perform. The way my easy AI creates a 
    // move is that it goes through every possible move for every single computer piece currently in the game 
    // and then verifies each and everyone one as their made. Once every valid move is made, one of the random 
    // moves are randomly chosen to be performed. Every single move is random and nothing influences the board.
    //
    // PARAMETERS:
    ...
    public void pawnPromotion(Piece[][] board, Move move){
        ...
        for(int i = 0; i < numPieces; i++){
            if(pieces[i].getPieceX() == move.getMoveX() && pieces[i].getPieceY() == move.getMoveY()){
                pieces[i].setPiece(move.getPiece());
            }
